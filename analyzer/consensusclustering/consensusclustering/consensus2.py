...
def compute_connectivity_matrix(labels: np.ndarray) -> np.ndarray:
    ...
    connectivity_matrix = np.equal.outer(labels, labels).astype(dtype)
...
def compute_identity_matrix(x: np.ndarray, resampled_indices: np.ndarray) -> np.ndarray:
    ...
    """
    n = x.shape[0]
    identity_matrix = np.zeros((n, n), dtype=dtype)
...
def compute_abs_difference_matrix(x: np.ndarray, y: np.ndarray, resampled_indices: np.array) -> np.ndarray:
    n = x.shape[0]
    abs_difference_matrix = np.zeros((n, n), dtype=dtype)
    abs_difference_matrix = np.abs(x - y)
    not_sampled_indices = np.setdiff1d(np.arange(n), resampled_indices)
    rows, cols = zip(*list(combinations(not_sampled_indices, 2)))
    abs_difference_matrix[rows, cols] = 0
    abs_difference_matrix[cols, rows] = 0
...
def compute_consensus_matrix(
    ...
    np.ndarray
        Consensus matrix
    """
    a = np.sum(connectivity_matrices, axis=0)
    b = np.sum(identity_matrices, axis=0)
    # replace 0 to float('nan')
    b = np.where(b == 0, np.nan, b)
    c = np.nan_to_num(a / b, nan=0)
    return c
...
class ConsensusClustering:
    ...
    def _fit_single_k(self, x: np.ndarray, k: int) -> np.ndarray:
        ...
        for _ in range(self.n_resamples):
            ...
            cm = compute_connectivity_matrix(labels)
            im = compute_identity_matrix(x, resampled_indices)
            ...
            adm = compute_abs_difference_matrix(cm, self.cohort_matrix, resampled_indices)
            connectivity_matrices.append(adm)
            identity_matrices.append(im)
    ...
    def fit(self, x: np.ndarray, y: list, progress_bar: bool = False, n_jobs: int = 0) -> None:
        """
        ...
        """
        n = y.shape[0]
        y_matrix = np.zeros((n, n), dtype=dtype)
        rows, cols = zip(*list(combinations(range(n), 2)))
        for i, j in zip(rows, cols):
            y_matrix[i, j] = y_matrix[j, i] = int(y[i] != y[j])
        for i in range(n):
            y_matrix[i, i] = 0
        self.cohort_matrix = y_matrix
        self.consensus_matrices_ = []
        if n_jobs == 0:
            for k in tqdm(
                self.cluster_range_,
