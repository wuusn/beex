"""
https://github.com/ccipd/MRQy/tree/master
...
def patient_name(root, folders_flag):
...
def volume_dicom(scans, name, folders_flag, middle_size, args, tag_list, tag_names):
...
def saveThumbnails_dicom(v, output, save_masks_flag, sample_size):
...
def saveThumbnails_mat(v, output, save_masks_flag):
    if save_masks_flag!='False':
        ffolder = output + '_foreground_masks'
        os.makedirs(ffolder + os.sep + v[1]['ID'], exist_ok=True)
    elif save_masks_flag=='False':
...
def worker_callback(s,csv_report):
...
def tsv_to_dataframe(tsvfileaddress):
    ...
    df = pd.read_csv(tsvfileaddress, sep='\t', skiprows=0, header=0, dtype=dtype_dict)
    return df
...
def run_qc(save_dir, cohort_dir):
    ...
    args = parser.parse_args([save_dir, cohort_dir])
    ...
    os.makedirs(fname_outdir, exist_ok=True)
    
    overwrite_flag = "w"        
    headers.append(f"outdir:\t{os.path.realpath(fname_outdir)}") 
    patients, names, dicom_spil, nondicom_spli, nondicom_names, mat_spli, mat_names = patient_name(root, folders_flag)
    ...
    if os.path.exists(address):
        os.remove(address)
    csv_report = open(address , overwrite_flag, buffering=1)
    first = True
    for i in range(len(names)):
        if dicom_flag:
            # print('debug:',dicom_spil)
            for j in range(len(dicom_spil)):
                v = volume_dicom(dicom_spil[j], names[j], middle_size, args, tag_list, tag_names)
                folder_foregrounds = saveThumbnails_dicom(v,fname_outdir, save_masks_flag, sample_size)
                s = BaseVolume_dicom(fname_outdir, v,j+1,folder_foregrounds, sample_size, ch_flag)
                if first and overwrite_flag == "w": 
                    first = False
                    csv_report.write("#dataset:"+"\t".join(s["output"])+"\n")
                worker_callback(s,csv_report)
            dicom_flag = False
        ...
        if nondicom_flag:
            for l,k in enumerate(nondicom_spli):
                ...
                s = BaseVolume_nondicom(fname_outdir, v,l+1, sample_size, ch_flag)
                if first and overwrite_flag == "w": 
                    first = False
                    csv_report.write("#dataset:"+"\t".join(s["output"])+"\n")
                worker_callback(s,csv_report)
        ...
        if mat_flag:
            for j in range(len(mat_spli)):
                ...
                folder_foregrounds = saveThumbnails_mat(v,fname_outdir, save_masks_flag)
